...
export default function TechStackSection() {
  return (
    <div className="relative overflow-hidden bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50 p-4 sm:p-6 lg:p-8 rounded-xl sm:rounded-2xl border border-white/20 shadow-lg backdrop-blur-sm">
      {/* Enhanced gradient fade effects */}
      <div className="absolute left-0 top-0 bottom-0 w-16 sm:w-24 lg:w-32 bg-gradient-to-r from-blue-50 via-blue-50/80 to-transparent z-10"></div>
      <div className="absolute right-0 top-0 bottom-0 w-16 sm:w-24 lg:w-32 bg-gradient-to-l from-purple-50 via-purple-50/80 to-transparent z-10"></div>
      
      <div className="text-center mb-6 sm:mb-8">
        <h3 className="text-xl sm:text-2xl lg:text-3xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent mb-2 sm:mb-3">
          Technologies I Work With
        </h3>
        <p className="text-gray-600 text-sm sm:text-base lg:text-lg">A comprehensive toolkit for building modern solutions</p>
      </div>
      ...
      <div className="flex">
        <div className="animate-marquee flex">
          {techStack.map((tech, index) => (
            <div key={`first-${index}`} className="inline-flex items-center mx-4 sm:mx-6 lg:mx-8 flex-shrink-0 group">
              <div className="relative p-1 sm:p-2">
                <Image 
                  ...
                  width={64}
                  height={64}
                  className="w-12 h-12 sm:w-14 sm:h-14 lg:w-16 lg:h-16 transition-all duration-300 group-hover:scale-110" 
                />
                ...
                <div className="absolute -top-8 sm:-top-10 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white px-2 sm:px-3 py-1 rounded-lg text-xs sm:text-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap hidden sm:block">
                  {tech.name}
                </div>
              </div>
            </div>
        ...
        <div className="animate-marquee flex" aria-hidden="true">
          {techStack.map((tech, index) => (
            <div key={`second-${index}`} className="inline-flex items-center mx-4 sm:mx-6 lg:mx-8 flex-shrink-0 group">
              <div className="relative p-1 sm:p-2">
                <Image 
                  src={tech.icon} 
                  alt={tech.name} 
